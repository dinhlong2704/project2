...
import com.example.projectfinal.databinding.CanthoNotehospitalFragBinding;
import com.example.projectfinal.databinding.DanangNotehospitalFragBinding;
...
public class CanThoHospitalFrag extends BaseFragment<CanthoNotehospitalFragBinding, M003HospitalVM> {
    public static final String TAG = CanThoHospitalFrag.class.getName();
    ...
    protected void initView() {
        ...
        try {
            viewModel.loadData(context.getAssets().open("hospitalDN.txt"));
        } catch (Exception e) {
    ...
    private void gotoListHospital() {
        showFragment(M004ListHospitalFrag.TAG, viewModel.getListHospital(),true);
    }
    ...
    @Override
    protected CanthoNotehospitalFragBinding initViewBinding(@NonNull LayoutInflater inflater, @Nullable ViewGroup container) {
        return CanthoNotehospitalFragBinding.inflate(getLayoutInflater());
    }
    ...
    public void showFragment(String tag, Object data,boolean isBack) {
        try {
            ...
            if (isBack) {
                trans.addToBackStack(M006ChonTinhFrag.TAG);//go back to previous screen
            }
            trans.commit();
        } catch (Exception e) {
            e.printStackTrace();
